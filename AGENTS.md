...
`lfpolicy` is a Python CLI and library for AWS Lake Formation permission review.
...
- Smoke review bundles after review contract changes:
  `PYTHONPATH=src python3 -m lfpolicy review --desired examples/desired.json --current-snapshot examples/current-snapshot.json --output-dir /tmp/lfpolicy-review-smoke --force`
...
- Do not make consuming-service IAM role design, approval identity checks, or
  runtime credential separation an `lfpolicy` package responsibility. `lfpolicy`
...
- Keep `setup.cfg`, `src/lfpolicy/_version.py`, `CHANGELOG.md`, and
...
- Keep the product boundary explicit: `lfpolicy` emits advisory evidence;
