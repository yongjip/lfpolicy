...
Use `lfpolicy` CI output as evidence that a Lake Formation policy change was
...
| --- | --- | --- |
| Generated desired state | `lfpolicy generate policy.py --output-file policy/desired.json --check` | Proves reviewed Python policy and committed desired JSON are in sync. |
| Review bundle | `lfpolicy review --desired policy/desired.json --current-snapshot snapshots/prod-current.json --output-dir artifacts/review` | Stable lint, audit, plan, planned grant evidence, input hashes, and summary status for approval systems. |
| Batch explain JSON | `lfpolicy explain-batch --requests access-requests.json --current-snapshot snapshots/prod-current.json --output json` | Answers operational access questions without opening the AWS console. |
...
```json
{
  "reviewed_plan": "artifacts/lfpolicy-plan.json",
...
different change set later. `lfpolicy` itself does not execute the changes.
